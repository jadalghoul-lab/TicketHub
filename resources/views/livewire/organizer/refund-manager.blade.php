<div class="max-w-screen-2xl mx-auto w-full p-6 space-y-8 bg-[#fdfdfd] min-h-screen">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
            ...
            <p class="text-slate-500 font-medium ml-5">Manage and process customer refund submissions</p>
        </div>
    </div>
                    ...
                    <tr class="group hover:bg-slate-50/50 transition-all">
                        <td class="py-6">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-bold text-slate-900">{{ $request->user->name }}</p>
                                    <p class="text-[10px] text-slate-400 font-bold">{{ $request->user->email }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-6">
                            <p class="text-sm font-bold text-slate-900">{{ $request->order->event->title }}</p>
                            <p class="text-xs text-indigo-600 font-black tracking-widest uppercase mt-1">{{ $request->order->order_number }}</p>
                        </td>
                        <td class="py-6">
                            <div class="max-w-xs">
                                <p class="text-xs text-slate-600 leading-relaxed italic">"{{ $request->reason }}"</p>
                            </div>
                        </td>
                        <td class="py-6">
                            @php
                        ...
                        <td class="py-6">
                            ...
                            <div class="flex gap-2">
                                ...
                                <button wire:click="reject({{ $request->id }})" class="bg-white text-red-600 border border-red-100 px-4 py-2 rounded-xl text-xs font-bold hover:bg-red-50 transition-all active:scale-95">
                                    Reject
                                </button>
                            </div>
                            @else
                    ...
                    <tr>
                        <td colspan="5" class="py-20 text-center">
                            <div class="flex flex-col items-center">
                                <span class="material-symbols-outlined text-6xl text-slate-200 mb-4">move_to_inbox</span>
                                <p class="text-slate-400 font-bold">No refund requests found.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
