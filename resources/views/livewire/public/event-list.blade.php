<div class="flex flex-col md:flex-row gap-8">
    ...
    <aside class="w-full md:w-[280px] shrink-0 space-y-6">
        <div class="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-100/50 dark:shadow-none sticky top-24">
            ...
            <div class="space-y-4 mb-8">
                <p class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Category</p>
                <div class="space-y-3">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" wire:model.live="categories" value="{{ $value }}"
                            class="rounded-lg border-slate-200 dark:border-slate-700 text-indigo-600 focus:ring-indigo-500 h-5 w-5 transition-all shadow-sm" />
                        <span class="text-sm font-bold text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{{ $label }}</span>
                    </label>
                    @endforeach
                </div>
            </div>
            ...
            <!-- Date Filter -->
            <div class="space-y-4 mb-8">
                <p class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">When</p>
                <select wire:model.live="dateRange"
                        class="w-full rounded-2xl border-none text-sm font-bold bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-4 focus:ring-indigo-500/20 transition-all p-4 shadow-inner">
            ...
            <div class="space-y-4 mb-8">
                <div class="flex justify-between items-center ml-1">
                    <p class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Max Price</p>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
            ...
            <div class="flex items-center gap-4">
                ...
                <select wire:model.live="sort"
                        class="rounded-2xl border-none text-xs font-black bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 focus:ring-4 focus:ring-indigo-500/20 shadow-xl shadow-slate-100/50 dark:shadow-none p-4 pr-10">
        ...
        <div wire:loading.remove wire:target="search, categories, dateRange, type, maxPrice, city, sort">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
                ...
                @php
                    ...
                    $categoryColors = [
                        'music'    => 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400',
                        'sports'   => 'bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-400',
                        'theater'  => 'bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400',
                        'festival' => 'bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400',
                    ];
                    $colorClass = $categoryColors[$event->category] ?? 'bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-400';
                @endphp
                <a href="{{ route('public.events.show', $event->slug) }}"
                   class="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-50 dark:border-slate-800 overflow-hidden shadow-2xl shadow-slate-100/50 dark:shadow-none hover:shadow-indigo-100 dark:hover:shadow-indigo-900/20 hover:-translate-y-2 transition-all duration-500 flex flex-col group relative">
                    ...
                    <div class="h-52 overflow-hidden relative">
                        ...
                        <div class="absolute top-4 left-4">
                            <span class="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter text-slate-900 dark:text-white shadow-lg">{{ $event->category }}</span>
                        </div>
                    </div>

                    <div class="p-6 flex flex-col flex-1">
                        <h4 class="text-xl font-black text-slate-900 dark:text-white mb-4 line-clamp-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{{ $event->title }}</h4>
                        
                        <div class="space-y-2 mb-6">
                            <div class="flex items-center gap-3 text-slate-500 dark:text-slate-400 text-[11px] font-bold">
                        ...
                        <div class="mt-auto flex items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-800">
                            <div>
                                <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-widest">From</p>
                                <p class="text-xl font-black text-indigo-600 dark:text-indigo-400">
                                    {{ $minPrice > 0 ? '€'.number_format($minPrice, 2) : 'Free' }}
                                </p>
                            </div>
                            <span class="bg-indigo-600 dark:bg-indigo-500 text-white px-5 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest shadow-lg shadow-indigo-100 dark:shadow-none group-hover:scale-105 transition-all">
                                Tickets
                            </span>
                        </div>
                    </div>
            ...
            @else
            <div class="text-center py-32 bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-50 dark:border-slate-800">
                <span class="material-symbols-outlined text-7xl text-slate-200 dark:text-slate-700 block mb-6">search_off</span>
                <h2 class="text-2xl font-black text-slate-900 dark:text-white mb-2">No matching events</h2>
                <p class="text-slate-500 dark:text-slate-400 font-medium mb-8">We couldn't find anything matching your current filters.</p>
