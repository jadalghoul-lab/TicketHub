<div class="relative w-full max-w-3xl mx-auto" x-data="{ open: false }" @click.away="open = false">
    <form wire:submit.prevent="performSearch" 
          class="bg-white/95 dark:bg-zinc-900/95 backdrop-blur shadow-2xl p-2 rounded-2xl flex flex-col md:flex-row gap-2 border border-slate-100 dark:border-zinc-800 transition-all duration-500 focus-within:ring-4 focus-within:ring-indigo-500/30 focus-within:border-indigo-500/50 focus-within:scale-[1.01] group outline-none">
        
        <div class="flex-grow flex items-center px-4 py-3 gap-3">
            <span class="material-symbols-outlined text-slate-400 group-focus-within:text-indigo-600 transition-colors">search</span>
        ...
        <div class="w-px bg-slate-200 dark:bg-zinc-800 hidden md:block my-2"></div>
        
        <div class="flex items-center px-4 py-3 gap-3">
            <span class="material-symbols-outlined text-slate-400 group-focus-within:text-indigo-600 transition-colors">location_on</span>
        ...
        <button type="submit"
                ...
                class="bg-indigo-600 text-white px-8 py-4 rounded-xl flex items-center justify-center gap-3 hover:bg-indigo-700 transition-all active:scale-95 font-black shadow-lg shadow-indigo-200 dark:shadow-none disabled:opacity-70 overflow-hidden relative group/btn">
    ...
    <div x-show="open && ($wire.search.length >= 2 || $wire.city.length >= 2)" 
         ...
         class="absolute top-full left-0 right-0 mt-4 bg-white dark:bg-zinc-900 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-zinc-800 overflow-hidden z-50 p-2"
         x-cloak>
        
        @if(count($results) > 0)
            <div class="p-4 border-b border-slate-50 dark:border-zinc-800">
            ...
            <div class="max-h-96 overflow-y-auto">
                @foreach($results as $event)
                    <a href="{{ route('public.events.show', $event->slug) }}" 
                       wire:navigate
                       class="flex items-center gap-4 p-4 hover:bg-slate-50 dark:hover:bg-zinc-800 transition-all group rounded-2xl">
                        <div class="w-16 h-12 bg-slate-100 dark:bg-zinc-800 rounded-lg overflow-hidden flex-shrink-0">
                            @if($event->image)
                        ...
                        <div class="flex-grow min-w-0">
                            <h4 class="font-bold text-slate-900 dark:text-white truncate group-hover:text-indigo-600 transition-colors">{{ $event->title }}</h4>
            ...
            <a href="{{ route('public.events.index', ['search' => $search, 'city' => $city]) }}" 
               wire:navigate
               class="block p-4 text-center bg-slate-50 dark:bg-zinc-800 text-xs font-black text-slate-500 uppercase tracking-widest hover:text-indigo-600 transition-all rounded-b-[2rem]">
        ...
        @else
            <div class="p-8 text-center">
                <div wire:loading.remove wire:target="search">
                    ...
                    <p class="text-slate-500 font-bold">No events found matching your search.</p>
                </div>
                <div wire:loading wire:target="search" class="flex flex-col items-center">
                    ...
                    <p class="text-slate-500 font-bold">Searching for "{{ $search }}"...</p>
                </div>
            </div>
        @endif
    </div>
