    ...
    <div class="max-w-screen-2xl mx-auto w-full p-6 space-y-8 bg-[#fdfdfd] min-h-screen">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div>
                ...
                <p class="text-slate-500 font-medium ml-5">{{ $organizer->company_name }} • Performance Overview</p>
            </div>
            ...
            <div class="flex items-center bg-white p-1 rounded-2xl border border-slate-100 shadow-sm">
                @foreach(['today' => 'Today', 'week' => 'Week', 'month' => 'Month'] as $key => $label)
                    <a href="?period={{ $key }}" class="px-6 py-2.5 rounded-xl text-sm font-bold transition-all {{ $period === $key ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-slate-600' }}">
                        {{ $label }}
                    </a>
                @endforeach
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <!-- Total Revenue -->
            <div class="bg-white p-6 rounded-[2rem] border border-slate-50 shadow-xl shadow-slate-100/50 flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    <div class="w-12 h-12 bg-green-50 rounded-2xl flex items-center justify-center">
                        <span class="material-symbols-outlined text-green-600">payments</span>
                    </div>
                    <span class="text-[10px] font-black text-green-600 bg-green-50 px-3 py-1 rounded-full uppercase">Live</span>
                </div>
                <div>
            ...
            <div class="bg-white p-6 rounded-[2rem] border border-slate-50 shadow-xl shadow-slate-100/50 flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-[10px] font-black text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full uppercase">{{ $period }}</span>
                </div>
                <div>
                    <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Tickets Sold</p>
            ...
            <div class="bg-white p-6 rounded-[2rem] border border-slate-50 shadow-xl shadow-slate-100/50 flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-[10px] font-black text-purple-600 bg-purple-50 px-3 py-1 rounded-full uppercase">Attendance</span>
                </div>
                <div>
                    <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Average Rate</p>
            ...
            <div class="bg-white p-6 rounded-[2rem] border border-slate-50 shadow-xl shadow-slate-100/50 flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-[10px] font-black text-amber-600 bg-amber-50 px-3 py-1 rounded-full uppercase">Active</span>
                </div>
                <div>
                    <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Total Events</p>
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-4 group hover:border-indigo-100 transition-all">
                            <div class="w-14 h-14 rounded-2xl bg-slate-900 flex items-center justify-center text-white font-black">
                                {{ substr($topEvent->title, 0, 1) }}
                            </div>
                            <div class="flex-grow">
                                <h4 class="font-bold text-slate-900 truncate w-40">{{ $topEvent->title }}</h4>
                                <p class="text-xs text-slate-400 font-bold">{{ $topEvent->tickets_count }} tickets sold</p>
                            </div>
                ...
                <!-- Recent Orders Table -->
                <div class="bg-white rounded-[2.5rem] p-8 border border-slate-50 shadow-xl shadow-slate-100/50">
                    <div class="flex items-center justify-between mb-8">
                        <h2 class="text-xl font-bold text-slate-900 flex items-center gap-2">
                            <span class="material-symbols-outlined text-indigo-600">history</span>
                            Recent Sales
                        </h2>
                        <button class="text-xs font-black text-indigo-600 uppercase tracking-widest hover:underline">View All</button>
                    </div>
                                ...
                                <tr class="group hover:bg-slate-50/50 transition-all">
                                    <td class="py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold text-[10px]">
                                    ...
                                    <td class="py-4">
                                        <p class="text-sm font-bold text-slate-900 truncate w-32">{{ $order->event->title }}</p>
                                        <p class="text-[10px] text-slate-400 font-bold">{{ $order->created_at->diffForHumans() }}</p>
                                    </td>
                                    <td class="py-4">
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white rounded-[2.5rem] p-8 border border-slate-50 shadow-xl shadow-slate-100/50">
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('organizer.scanner', $event->slug) }}" class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl group hover:bg-indigo-600 transition-all">
                            <span class="text-xs font-bold text-slate-700 group-hover:text-white transition-colors truncate w-40">{{ $event->title }}</span>
                            <span class="material-symbols-outlined text-slate-400 group-hover:text-white text-sm">arrow_forward_ios</span>
                        </a>
                        @endforeach
                    </div>
                </div>
